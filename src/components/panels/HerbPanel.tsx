  ...
  if (selecting) {
    ...
    return (
      <div className="bg-slate-800 border border-amber-600/40 rounded-lg p-3 flex flex-col gap-2 min-h-[120px]">
        ...
        {availableHerbs.length === 0 ? (
          ...
          availableHerbs.map(herb => (
            <button
              type="button"
              key={herb.id}
              onClick={() => { onPlant(plot.id, herb.id); setSelecting(false); }}
        ...
        <button type="button" onClick={() => setSelecting(false)} className="text-xs text-slate-500 hover:text-slate-300 cursor-pointer">取消</button>
      </div>
    );
  }
