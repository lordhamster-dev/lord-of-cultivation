...
export function FishingPanel() {
    ...
    const activeArea = fishing.currentAreaId ? FISHING_AREAS.find((a) => a.id === fishing.currentAreaId) : null;
                ...
                <div className="space-y-3">
                    ...
                    <div className="bg-slate-800 border border-slate-700 rounded-lg p-3">
                        ...
                        <div className="flex flex-wrap gap-1">
                            {activeArea.fish
                                .filter((f) => f.minLevel <= fishingLevel)
                                .map((f) => (
                                    <span
                                        key={f.id}
                            ...
                            {activeArea.specialItems.map((s) => {
                                ...
                                return item ? (
                                    <span
                                        key={s.itemId}
                                        className="text-xs bg-amber-900/30 px-2 py-0.5 rounded text-amber-400"
                                    >
                                        {item.emoji}
                                    ...
                                    >
                                        ...
                                        {!unlocked ? (
                                            <div className="text-xs text-slate-500 mt-0.5">
                                                需要 Lv.{area.requiredLevel}
                                            </div>
                                        ) : (
                                            <div className="text-xs text-slate-500 mt-0.5">
                                                {area.fish.filter((f) => f.minLevel <= fishingLevel).length} 种鱼
                                            </div>
                                        )}
                                    </button>
                                            ...
                                            <div className="flex flex-wrap gap-1">
                                                ...
                                                {area.fish
                                                    ...
                                                    .map((f) => (
                                                        <span
                                                            key={f.id}
                                                            className="text-xs bg-slate-700/50 px-2 py-0.5 rounded text-slate-500 line-through"
                                                        >
