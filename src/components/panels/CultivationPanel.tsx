...
import { getFullStageName, getSubStageCount, STAGES } from "../../core/data/stages";
import { TECHNIQUES } from "../../core/data/techniques";
import type { RealmTier } from "../../core/types";
import {
    selectBreakthroughCost,
    selectBreakthroughCostWithoutPill,
    selectHasBreakthroughPill,
    selectIsAtFinalSubStage,
    selectSpiritStones,
    selectStage,
    useGameStore,
...
export function CultivationPanel() {
    ...
    const [realmJourneyOpen, setRealmJourneyOpen] = useState(false);

    const subStageCount = getSubStageCount(stageIndex);

    const canBreakthrough =
        progress >= 100 && atFinalSubStage && spiritStones >= breakthroughCost && stageIndex < STAGES.length - 1;
    ...
    return (
        <div className="p-4 space-y-6">
            ...
            <div className="bg-slate-800 rounded-lg p-4">
                <button
                    onClick={() => setRealmJourneyOpen((o) => !o)}
                    className="w-full flex items-center justify-between cursor-pointer"
                >
                    <h3 className="text-slate-300 text-sm font-semibold">境界历程</h3>
                    <span className="text-slate-500 text-xs">{realmJourneyOpen ? "▲ 收起" : "▼ 展开"}</span>
                </button>
                {realmJourneyOpen && (
            ...
            <div className="bg-slate-800 rounded-lg p-4 space-y-2">
                <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-purple-300">
                        {spirit.toFixed(1)} / {spiritMax}
                    </span>
                </div>
                <ProgressBar value={(spirit / spiritMax) * 100} color="bg-purple-500" />
                <div className="text-xs text-slate-500 text-right">基础恢复: +{spiritPerSec.toFixed(1)}/秒</div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-slate-800 rounded-lg p-4 flex flex-col">
                    <div className="flex items-center justify-between">
                        ...
                        {meditationActive && <span className="text-xs text-purple-300 animate-pulse">打坐中...</span>}
                    </div>
                    <div className="text-xs text-slate-400 space-y-1 mt-3 flex-1">
                            ...
                            {!atFinalSubStage && (
                                <p className="text-xs text-slate-400">
                                    ...
                                    <span className="text-amber-300">
                                        {stage?.subStages[subStageCount - 1]?.name ?? "最终"}
                                    </span>{" "}
                                    才可大突破
                                </p>
                            )}
                                ...
                                <div className="flex items-center justify-between text-sm">
                                    ...
                                    {hasPill ? (
                                        ...
                                        <span className="text-slate-500">
                                            未持有 (可减少{(nextStage.breakPillDiscount * 100).toFixed(0)}%灵石)
                                        </span>
                                    )}
                                </div>
                            )}
            ...
            <div className="bg-slate-800 rounded-lg p-4 space-y-3">
                ...
                <div className="text-xs text-slate-400 mb-2">功法可在打坐时提升灵力恢复效率</div>
