...
import { getTotalAchievements, getUnlockedCount } from "../../core/systems/AchievementSystem";
            ...
            <div className="space-y-2">
                {visible.map((ach) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`text-2xl ${isUnlocked ? "" : "grayscale opacity-40"}`}>{ach.icon}</div>
                            <div className="flex-1">
                                <div
                                    className={`font-medium text-sm ${isUnlocked ? "text-amber-300" : "text-slate-400"}`}
                                >
                                    {ach.name}
                                </div>
                                <div className="text-xs text-slate-500 mt-0.5">{ach.description}</div>
                            </div>
                            {isUnlocked && <div className="text-green-400 text-xs font-bold">✓ 已解锁</div>}
                        </div>
                    );
                })}
                {visible.length === 0 && <div className="text-center text-slate-500 text-sm py-8">该分类暂无成就</div>}
            </div>
        </div>
    );
}
