...
import { canCraft, getActualFailChance, getAlchemyDuration } from "../../core/systems/AlchemySystem";
...
export function AlchemyPanel() {
    ...
    const activeRecipe = alchemy.currentRecipeId ? ALCHEMY_RECIPES.find((r) => r.id === alchemy.currentRecipeId) : null;
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {ALCHEMY_RECIPES.map((recipe) => {
                            const locked =
                                alchemyLevel < recipe.alchemyLevelRequired ||
                            ...
                            return (
                                <div key={recipe.id}>
                                    <button
                                        onClick={() => !locked && setSelectedRecipe(isSelected ? "" : recipe.id)}
                                    ...
                                    >
                                        ...
                                        {locked ? (
                                            <div className="text-xs text-slate-500 mt-0.5">
                                                ...
                                                {recipe.cultivationStageRequired > 0 &&
                                                    ` / ${STAGES[recipe.cultivationStageRequired]?.name ?? ""}`}
                                            </div>
                                        ) : (
                                            <div
                                                className={`text-xs mt-0.5 ${craftable ? "text-green-400" : "text-red-400"}`}
                                            >
                                                {craftable ? "可炼制" : "材料不足"}
                                            </div>
                                        )}
                                    ...
                                    {isSelected && !locked && (
                                        <div className="mt-1 bg-slate-800/50 rounded-lg p-3 space-y-2 border border-slate-700 col-span-2">
                                            <div className="space-y-1">
                                                ...
                                                {recipe.ingredients.map((ing) => {
                                                    ...
                                                    return (
                                                        <div
                                                            key={ing.itemId}
                                                            className={`text-xs flex justify-between ${enough ? "text-slate-300" : "text-red-400"}`}
                                                        >
                                            ...
                                            <div className="text-xs text-slate-500">
                                                失败率: {(getActualFailChance(recipe, alchemyLevel) * 100).toFixed(0)}%
                                                | 产出: {recipe.outputQuantity[0]}-{recipe.outputQuantity[1]} 颗
                                            </div>
                                            <Button
                                                variant="primary"
                                                onClick={() => startAlchemy(recipe.id)}
                                                className="w-full"
                                                disabled={
                                                    activeActivity !== null ||
                                                    alchemyLevel < recipe.alchemyLevelRequired ||
