...
import type { CombatStats, DungeonState, Inventory, ItemStack } from "../types";
...
export function canEnterDungeon(state: DungeonState, dungeonId: string, stageIndex: number): boolean {
...
/** Start a dungeon run */
export function startDungeon(state: DungeonState, dungeonId: string, playerMaxHp: number): DungeonState {
    const dungeon = getDungeon(dungeonId);
