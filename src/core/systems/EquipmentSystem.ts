import type { EquipmentState, Inventory, EquipmentSlotId, EquipmentInstance } from '../types';
import { getEquipment, getEnhanceCost, getEnhanceMaterials, getEquipmentTotalStats } from '../data/equipment';
...
export function canForge(
  ...
  return equipDef.forgingRecipe.ingredients.every(
    ing => (inventory.items[ing.itemId] ?? 0) >= ing.quantity,
  );
}
...
export function canEnhance(
  ...
  return materials.every(
    mat => (inventory.items[mat.itemId] ?? 0) >= mat.quantity,
  );
}
