...
export function migrate(raw: Partial<GameState>): GameState {
    ...
    if (data.version < 5) {
        ...
        if (data.inventory?.items) {
            ...
            if (oldPillCount > 0) {
                data.inventory.items["core_formation_pill"] =
                    (data.inventory.items["core_formation_pill"] ?? 0) + oldPillCount;
    ...
    if (data.version < 6) {
        ...
        const oldSpiritStones = data.resources?.spiritStones ? Math.floor(parseFloat(data.resources.spiritStones)) : 0;
