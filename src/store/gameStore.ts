...
import { getItem, PILL_COMBAT_EFFECTS } from "../core/data/items";
import { getRecipe } from "../core/data/recipes";
import { MAX_STAGE_INDEX, STAGES } from "../core/data/stages";
...
import { canCraft, tickAlchemy } from "../core/systems/AlchemySystem";
import { createInitialCombatState, getPlayerCombatStats, tickCombat } from "../core/systems/CombatSystem";
import {
    attemptBreakthrough,
    computeProgressGain,
    getBreakthroughCost,
...
import {
    ...
    refreshDungeonDaily,
    startDungeon as startDungeonSystem,
    tickDungeon,
} from "../core/systems/DungeonSystem";
import {
    canEnhance,
    canForge,
    createInitialEquipmentState,
    enhanceEquipment as enhanceEquipmentSystem,
    forgeEquipment as forgeEquipmentSystem,getEquipmentBonuses 
} from "../core/systems/EquipmentSystem";
import { tickFishing } from "../core/systems/FishingSystem";
import {
    harvestHerb as harvestHerbSystem,
    plantHerb as plantHerbSystem,
    tickHerbGrowth,
} from "../core/systems/HerbSystem";
import { computeMeditationRegenRate, computeSpiritStats, getCombatSpiritCost } from "../core/systems/ProductionSystem";
import {
    claimQuestReward,
    createInitialQuestState,
    refreshDailyQuestsIfNeeded,
    updateQuestProgress,
} from "../core/systems/QuestSystem";
import { applyExpGain, getSpiritStoneCount } from "../core/systems/ResourceSystem";
import { addSkillExp, createInitialSkillState } from "../core/systems/SkillSystem";
import type { ActivityType, CombatSupplyState, EquipmentSlotId, GameState, HerbPlot } from "../core/types";
...
export const useGameStore = create<GameStore>()(
    immer((set, get) => ({
            ...
            set((state) => {
                ...
                if (state.combat.isActive) {
                    ...
                    if (newSpirit <= 0) {
                        ...
                        if (combatResult.spiritStonesGain > 0) {
                            ...
                            state.inventory.items["spirit_stone"] =
                                (state.inventory.items["spirit_stone"] ?? 0) + combatResult.spiritStonesGain;
                            // Track daily quest
                            state.dailyQuests = updateQuestProgress(state.dailyQuests, {
                                spiritStonesEarned: combatResult.spiritStonesGain,
                            });
                        }
                        ...
                        const combatPlayerStats = getPlayerCombatStats(
                            state.cultivation.stageIndex,
                            state.equipment,
                            state.skills.combat.level,
                        );
                        ...
                        if (supplyConfig.hpItemId && state.combat.isActive && state.combat.playerHp > 0) {
                            ...
                            if (hpPercent < supplyConfig.hpThreshold && supply.hpItemsUsed < supplyConfig.hpItemCount) {
                                ...
                                if (hpItemQty > 0) {
                                    ...
                                    const hpRestoreRatio = pillEffect ? pillEffect.hpRecovery : 0.3;
                                    state.combat.playerHp = Math.min(
                                        combatPlayerStats.maxHp,
                                        state.combat.playerHp + combatPlayerStats.maxHp * hpRestoreRatio,
                                    );
                        ...
                        if (supplyConfig.spiritItemId && state.combat.isActive) {
                            ...
                            if (
                                spiritPercent < supplyConfig.spiritThreshold &&
                                supply.spiritItemsUsed < supplyConfig.spiritItemCount
                            ) {
                                ...
                                if (spiritItemQty > 0) {
                                    ...
                                    const spiritRestoreRatio = pillEffect ? pillEffect.spiritRecovery : 0.3;
                                    state.resources.spirit = Math.min(
                                        state.resources.spiritMax,
                                        state.resources.spirit + state.resources.spiritMax * spiritRestoreRatio,
                                    );
                ...
                if (state.dungeon.isActive) {
                    ...
                    if (newSpirit <= 0) {
                        ...
                        const dungeonResult = tickDungeon(state.dungeon, playerStats, state.inventory, deltaMs);
                        ...
                        if (dungeonResult.spiritStonesGain > 0) {
                            state.inventory.items["spirit_stone"] =
                                (state.inventory.items["spirit_stone"] ?? 0) + dungeonResult.spiritStonesGain;
                            state.dailyQuests = updateQuestProgress(state.dailyQuests, {
                                spiritStonesEarned: dungeonResult.spiritStonesGain,
                            });
                        }
                        ...
                        const dungeonPlayerStats = getPlayerCombatStats(
                            state.cultivation.stageIndex,
                            state.equipment,
                            state.skills.combat.level,
                        );
                        ...
                        if (dConfig.hpItemId && state.dungeon.isActive && state.dungeon.playerHp > 0) {
                            ...
                            if (hpPct < dConfig.hpThreshold && dSupply.hpItemsUsed < dConfig.hpItemCount) {
                                ...
                                if (qty > 0) {
                                    ...
                                    const hpRestoreRatio = pillEffect ? pillEffect.hpRecovery : 0.3;
                                    state.dungeon.playerHp = Math.min(
                                        dungeonPlayerStats.maxHp,
                                        state.dungeon.playerHp + dungeonPlayerStats.maxHp * hpRestoreRatio,
                                    );
                                    state.combatSupply.hpItemsUsed += 1;
                                }
                            }
                ...
                const fishingResult = tickFishing(state.fishing, fishingLevel, state.inventory, deltaMs);
        ...
        startFishing(areaId: string): void {
            ...
            set((draft) => {
                draft.fishing = {
                    ...
                    progressMs: 0,
                    totalFishCaught: draft.fishing.totalFishCaught,
                };
        ...
        startAlchemy(recipeId: string): void {
            ...
            set((draft) => {
                draft.alchemy = {
                    ...
                    progressMs: 0,
                    totalPillsCrafted: draft.alchemy.totalPillsCrafted,
                };
        ...
        unlockHerbPlot(plotIndex: number): boolean {
            ...
            set((draft) => {
                ...
                if (cost.spiritStones > 0) {
                    draft.inventory.items["spirit_stone"] =
                        (draft.inventory.items["spirit_stone"] ?? 0) - cost.spiritStones;
                }
            });
            return true;
        },
        ...
        forgeEquipment(equipDefId: string): boolean {
            ...
            if (!canForge(def, state.inventory, spiritStones, state.skills.forging.level, state.cultivation.stageIndex))
                return false;
            ...
            set((draft) => {
                ...
                // Deduct spirit stones from inventory
                draft.inventory.items["spirit_stone"] =
                    (draft.inventory.items["spirit_stone"] ?? 0) - result.spiritStonesCost;
...
export const selectHasBreakthroughPill = (state: GameStore) =>
    hasBreakthroughPill(state.cultivation.stageIndex, state.inventory);
